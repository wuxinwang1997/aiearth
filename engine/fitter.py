...
class Fitter:
    def __init__(self, model, ema, device, cfg, train_loader, val_loader, logger):
        self.config = cfg
        self.epoch = 0
    ...
    def validation(self):
        ...
        with torch.no_grad():
            for step, (sst, labels) in enumerate(valid_loader):
                ...
                loss1, loss2, loss3 = self.loss(outputs[0], labels), self.loss(outputs[1], labels), self.loss(outputs[2], labels)
                loss = (loss1+loss2+loss3)/3
                summary_loss.update(loss.item(), sst.shape[0])
                y_pred.append(torch.mean(torch.cat(outputs, dim=0), dim=1))
